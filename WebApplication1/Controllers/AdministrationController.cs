    ...
    public class AdministrationController : Controller
    {
        ...
        public async Task<IActionResult> ManageUserClaims(string userId)
        {
            ...
            foreach(Claim claim in ClaimsStore.AllClaims)
            {
                ...
                userClaim.IsSelected = existingClaims.Any(c => c.Type == claim.Type && c.Value=="true");
        ...
        public async Task<IActionResult> ManageUserClaims(UserClaimsViewModel model)
        {
            ...
            for (int i = 0; i < model.Claims.Count; i++)
            {
                ...
                {
                    Claim holder = new Claim(model.Claims[i].ClaimType, model.Claims[i].IsSelected?"true":"false");
                    result = await userManager.AddClaimAsync(user, holder);
                }
            }
            if (!result.Succeeded)
            {
